              ...
              <DestinationCardMap
                ...
                transportEndMarker={
                  destination.transport?.end_latitude != null && destination.transport?.end_longitude != null
                    ? { lat: destination.transport.end_latitude, lon: destination.transport.end_longitude, name: `${destination.transport.type ?? 'Transport'} → ${destination.name}`, type: destination.transport.type, memo: destination.transport.memo }
                    : null
                }
                transportStartMarker={(() => {
                  ...
                  return next?.transport?.start_latitude != null && next?.transport?.start_longitude != null
                    ? { lat: next.transport.start_latitude, lon: next.transport.start_longitude, name: `${next.transport.type ?? 'Transport'} → ${next.name}`, type: next.transport.type, memo: next.transport.memo }
                    : null;
                })()}
              />
            )}
