...
export function ReadonlyDestinationModal({ dest, nextDest, onClose }: {
  dest: DestinationWithTransport;
  nextDest: DestinationWithTransport | null;
  onClose: () => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center" onMouseDown={handleClose}>
      ...
      <div
        className={`relative z-10 flex h-full w-full flex-col overflow-hidden bg-white shadow-xl dark:bg-zinc-800 transition-all duration-200 ease-out sm:mx-4 sm:h-auto sm:max-h-[80vh] sm:w-full sm:max-w-sm sm:rounded-lg sm:border sm:border-zinc-200 dark:sm:border-zinc-700 ${visible ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0 sm:translate-y-8'}`}
